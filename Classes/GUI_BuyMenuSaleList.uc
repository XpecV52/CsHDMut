...
function int PopulateBuyables()
{
    ...
    for(j = 0;j < KFLR.ItemForSale.Length;j++)
    {
        ...
        if(class'CsHDMut'.static.IsSingleHandgun(WP.default.InventoryType))
        {
            if(IsInInventory(class'CsHDMut'.static.DualPickupOf(WP)))
            {
                // goto J0x4A7;
            return;
            }
        ...
        {
            if(class'CsHDMut'.static.IsDualHandguns(WP.default.InventoryType))
            {
                if(IsInInventory(class'CsHDMut'.static.SinglePickupOf(WP)))
                {
                    DualDivider = 2;
                }
            }
        ...
        if(DualDivider == 2)
        {
            ForSaleBuyable.ItemWeight = float(Min(0, int(WP.default.Weight - class'CsHDMut'.static.SinglePickupOf(WP).default.Weight)));
        }
        else
        {
