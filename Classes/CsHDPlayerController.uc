...
function SelectVeterancy(class<KFVeterancyTypes> VetSkill, optional bool bForceChange)
{
    ...
    else
    {
        if(!bChangedVeterancyThisWave || bForceChange)
        {
            ...
            PRI.ClientVeteranSkill = class'CsHDMut'.static.GetVetReplacement(VetSkill);
...
simulated function PreloadFireModeAssets(class<WeaponFire> FClass)
{
    ...
    {
        if(class<KFShotgunFire>(FClass) != none)
        {
            class<KFShotgunFire>(FClass).static.PreloadAssets(Level);
        }
        else
        {
            if(class<KFMeleeFire>(FClass) != none)
            {
    ...
    {
        if((class<HealingProjectile>(pClass) != none) && class<HealingProjectile>(pClass).default.StaticMeshRef != "")
        {
            class<HealingProjectile>(pClass).static.PreloadAssets();
        }
        else
        {
            if((class<LAWProj>(pClass) != none) && class<LAWProj>(pClass).default.StaticMeshRef != "")
            {
                class<LAWProj>(pClass).static.PreloadAssets();
            }
            else
            {
                if((class<M79GrenadeProjectile>(pClass) != none) && class<M79GrenadeProjectile>(pClass).default.StaticMeshRef != "")
                {
                    class<M79GrenadeProjectile>(pClass).static.PreloadAssets();
                }
                else
                {
                    if((class<SealSquealProjectile>(pClass) != none) && class<SealSquealProjectile>(pClass).default.StaticMeshRef != "")
                    {
                        class<SealSquealProjectile>(pClass).static.PreloadAssets();
                    }
                    else
                    {
                        if((class<SPGrenadeProjectile>(pClass) != none) && class<SPGrenadeProjectile>(pClass).default.StaticMeshRef != "")
                        {
                            class<SPGrenadeProjectile>(pClass).static.PreloadAssets();
                        }
                        else
                        {
                            if((class<NailGunProjectile>(pClass) != none) && class<NailGunProjectile>(pClass).default.StaticMeshRef != "")
                            {
                                class<NailGunProjectile>(pClass).static.PreloadAssets();
                            }
                            else
                            {
                                if((class<TrenchgunBullet>(pClass) != none) && class<TrenchgunBullet>(pClass).default.StaticMeshRef != "")
                                {
                                    class<TrenchgunBullet>(pClass).static.PreloadAssets();
                                }
                                else
                                {
                                    if((class<CrossbowArrow>(pClass) != none) && class<CrossbowArrow>(pClass).default.MeshRef != "")
                                    {
                                        class<CrossbowArrow>(pClass).static.PreloadAssets();
                                    }
                                    else
                                    {
                                        if(class<M99Bullet>(pClass) != none)
                                        {
                                            class<M99Bullet>(pClass).static.PreloadAssets();
                                        }
                                        else
                                        {
                                            if((class<PipeBombProjectile>(pClass) != none) && class<PipeBombProjectile>(pClass).default.StaticMeshRef != "")
                                            {
